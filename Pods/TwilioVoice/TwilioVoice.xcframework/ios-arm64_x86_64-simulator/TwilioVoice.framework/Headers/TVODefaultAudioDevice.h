...
static TVOAVAudioSessionConfigurationBlock _Nonnull kTVODefaultAVAudioSessionConfigurationBlock = ^() {
    ...
    if ([session respondsToSelector:@selector(setCategory:mode:options:error:)]) {
        if (![session setCategory:AVAudioSessionCategoryPlayAndRecord
                          ...
                          options:AVAudioSessionCategoryOptionAllowBluetoothA2DP | AVAudioSessionCategoryOptionAllowBluetoothHFP
#else
...
/**
 * @brief A boolean to ignore preferred configuration errors.
 *
 * @discussion When YES, calls to AudioSession preferred configuration ignore errors in
